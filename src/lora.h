...
do { channel = (uint32_t) ((((uint64_t) freq)<<25)/(XTAL_FREQ) ); } while( 0 )
...
subghz_result_t subghz_set_cad_params(subghz_cad_symbols_t nb_symbols, uint8_t det_peak, uint8_t det_min, subghz_cad_exit_mode_t exit_mode, uint32_t timeout);
...
subghz_result_t subghz_config_dio_irq(uint16_t irq_mask, uint16_t irq1_mask, uint16_t irq2_mask, uint16_t irq3_mask);
